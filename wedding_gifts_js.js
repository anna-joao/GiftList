...
async function loadSelectedGifts() {
    try {
        const { getFirestore, collection, getDocs } = await import("https://www.gstatic.com/firebasejs/12.8.0/firebase-firestore.js");
        ...
        querySnapshot.forEach((doc) => {
            const data = doc.data();
            // Só adiciona se estiver ocupado
            if (data.occupied) {
                selectedGifts.push({
                    giftId: data.giftId,
...
// Atualizar presente no Firestore
async function updateSelectedGift(giftId, guestName) {
    try {
        const { getFirestore, doc, updateDoc, getDoc } = await import("https://www.gstatic.com/firebasejs/12.8.0/firebase-firestore.js");
        const db = getFirestore();
        
        // Usa o giftId como ID do documento
        const giftDocRef = doc(db, 'selectedGifts', giftId.toString());
        
        // Verifica se já está ocupado
        const docSnap = await getDoc(giftDocRef);
        if (docSnap.exists() && docSnap.data().occupied) {
            alert('Este presente já foi escolhido por outra pessoa!');
            await loadSelectedGifts(); // Recarrega a lista
            return false;
        }
        ...
        console.log('Presente atualizado com sucesso!');
        return true;
    } catch (error) {
...
async function confirmGift() {
    ...
    const success = await updateSelectedGift(selectedGift.id, guestName);
